...
var add = function(entity) {
	...
	$("select[id$=item-producto-list] > option").remove();
	...
	/*if (entity == ENTITY_ITEM) {
		...
		populateSelectBox('item-product-list', '/producto');
	}*/
}
	...
	var successFn=function(resp){
		...
		if(data.length > 0){
			for (var i=0;i<data.length;i++){
				...
				{
				case ENTITY_PRODUCT:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].descripcion+'</td>';
					break;
				case ENTITY_ITEM:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].price+'</td><td>'+data[i].producto+'</td>';
					break;
				default:
					htm+=""; 
				}
				if(entity == ENTITY_ITEM)
					htm+='<td><a href="#" class="delete-entity" onclick=\'deleteEntity("'+entity+'","'+data[i].name+'","'+data[i].producto+'")\'>Delete</a> | <a href="#" class="edit-entity" onclick=\'edit("'+entity+'","'+data[i].name+'")\'>Edit</a></td></tr>';
				else
